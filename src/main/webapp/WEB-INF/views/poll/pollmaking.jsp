         ...
         <div class="col-md-12">
            ...
            <div class="box box-primary">
               <div class="box-header with-border">
                  <h3 class="box-title">질문 생성 페이지</h3>
               </div>
               <!-- /.box-header -->
                                  
              <!-- /.box-body -->
               <div class="box-body">
                 <form class='registerForm' role="form" action="/poll/pollmaking" method="post">
                   <input type="hidden" name="pno" value="${pollVO.pno}">
                   <div class="form-group">
                     <label>주제</label> <input type="text" id="topic" class="form-control" name="topic" placeholder="주제...">
                   </div>
                   <div class="form-group">
                     <label>설명</label>
                     <textarea class="form-control" rows="3" name="intro" placeholder="설명..."></textarea>
                   </div>
                   <div class="form-group">
              ...
              <form action="/poll/uploadForm" method="post" enctype="multipart/form-data" target="zeroFrame">
               ...
               <small id="itemFile"></small><button id="fileDelete" class="btn btn-box-tool"><i class="fa fa-times"></i></button>
             </form>
           ...
           <div class="box-footer">
              <a href="/poll/list" class="btn btn-default pull-left" id="cancel_btn">등록취소</a>
              <button type="submit" id="registBtn" class="btn btn-info pull-right">등록하기</button>
           </div>
            <!-- /.box -->
         </div>
         <!--/.col (right) -->
...
<script>
  ...
  if('${pollVO.answerCode}' != 'answer1'){
   ...
   $('#divItem').append(item).append(item);
  }
  //항목추가
...
//파일 첨부시 파일전송 버튼 눌렀을때 해당 항목 파일 전송
  ...
  $(document).on("click","#fileDelete",function(event){
	  ...
	  var $file = $(this).siblings('#itemFile');
...
//등록하기 버튼 클릭시 항목의 텍스트와 파일명을 히든태그로 만들어서 form태그 전송. 항목이 비어있으면 전송안됨.
